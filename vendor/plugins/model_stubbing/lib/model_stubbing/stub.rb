        ...
        if value.is_a? Stub
          if defined?(ActiveRecord)
            if reflection = model_class.reflect_on_association(key)
              $stderr.puts "Reflection '#{key}' found for #{model_class.name} while guessing column_name"
              reflection.primary_key_name
            else
              debugger
              $stderr.puts "No reflection '#{key}' found for #{model_class.name} while guessing column_name"
